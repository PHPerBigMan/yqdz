                        ...
                        <table class="am-table am-table-striped am-table-hover table-main">
                            ...
                            @foreach($data as $v)
                                <tr>
                                    ...
                                    <td>{{ $v->title }}</td>
                                    <td>{{ $v->name }}</td>
                                    <td><img :src="{{ $v->carouselimg }}" width="100" height="50"> </td>
                                    <td>{{ $v->url }}</td>
                                    <td>
                                        <div class="am-btn-toolbar">
                                            <div class="am-btn-group am-btn-group-xs">
                                                <a type="button" class="am-btn am-btn-success am-btn-sm" @click="edit({{$v->carouselid}})"><span class="am-icon-pencil-square-o"></span>  编辑</a>
                                                <a type="button" class="am-btn am-btn-danger am-btn-sm" @click="shanchu({{$v->carouselid}},this)"><span class="am-icon-trash-o"></span>  删除</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
<script>
    var vm = new Vue({
        ...
        methods: {
            add: function (id) {
                layer_show("添加广告轮播",'add/',600,500)
            },
            edit: function (id) {
                layer_show("修改广告轮播",'edit/?id='+id,600,500)
            },
