...
<div class="hw-content" style="padding: 20px;">
    <form action="/admin/classify/handle" method="post" class="am-form" id="form-admin-add" enctype="multipart/form-data">
        ...
        <div class="am-g am-margin-top">
            <div class="am-u-sm-3 am-u-md-2 am-text-right">分类类型</div>
            <div class="am-u-sm-8 am-u-md-8">
                <select name="type" id="">
                    <option value="0" <?php if(isset($data->type)){if($data->type == "产品分类") echo "selected";}?>>产品分类</option>
                    <option value="1" <?php if(isset($data->type)){if($data->type == "定制分类") echo "selected";}?>>定制分类</option>
                </select>
            </div>
            <div class="am-hide-sm-only am-u-md-6"></div>
        </div>
